...
def display_name(col): return DISPLAY_MAP.get(col, col)
...
if mode in ["처리별 VOC 비교", "시간별 VOC 변화"]:
    ...
    if mode == "처리별 VOC 비교":
            ...
            if show_anova:
                ...
                if anova_df[TREAT_COL].nunique() >= 2 and all(anova_df.groupby(TREAT_COL)[selected_voc_internal].count() >= 2):
                    ...
                    if "Tukey HSD" in posthoc_choices:
                        ...
                        st.download_button("⬇️ Tukey HSD 테이블 CSV", data=tukey_df.to_csv(index=False).encode("utf-8-sig"),
                                           file_name="tukey_hsd_results.csv", mime="text/csv")
                    ...
                    if "Duncan" in posthoc_choices:
                        if HAS_SCPH:
                            try:
                                ...
                                st.download_button("⬇️ Duncan p-value 매트릭스 CSV", data=duncan_mat.to_csv().encode("utf-8-sig"),
                                                   file_name="duncan_posthoc_pvals.csv", mime="text/csv")
                            except Exception as e:
                    ...
                    # letters를 그래프에 그리기 (facet 미사용 시, color 그룹 유무와 무관)
                    ...
                    if summary_tables:
                        ...
                        for label, tbl in summary_tables:
                            st.markdown(f"**{label}**")
                            st.dataframe(tbl)
                    st.download_button("⬇️ ANOVA 테이블 CSV", data=anova_table.to_csv().encode("utf-8-sig"),
        ...
        if PROGRESS_COL in data_use.columns: fig_kwargs["color"] = PROGRESS_COL
        ...
        if cols_exist:
            for env_col in cols_exist:
                ...
                if PROGRESS_COL in data_use.columns: group_keys_env.append(PROGRESS_COL)
                if CHAMBER_COL in data_use.columns and facet_by_chamber: group_keys_env.append(CHAMBER_COL)
                if LINE_COL in data_use.columns and facet_by_line: group_keys_env.append(LINE_COL)
                ...
                if PROGRESS_COL in data_use.columns: fig_kwargs_env["color"] = PROGRESS_COL
...
# -------------------------
    ...
    if not results:
        ...
        st.download_button("⬇️ 스크리닝 결과 CSV", data=res_df.to_csv(index=False).encode("utf-8-sig"),
                           file_name="voc_screening_results.csv", mime="text/csv")
