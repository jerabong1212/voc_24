...
if mode == "처리별 VOC 비교":
    ...
    color_kw = {"color": PROGRESS_COL} if PROGRESS_COL in data_use.columns and PROGRESS_COL in data_use and data_use[PROGRESS_COL].notna().any() else {}
    ...
    if chart_type == "막대그래프":
        ...
        if show_anova:
            ...
            if anova_df[TREAT_COL].nunique() >= 2 and all(
                ...
                means_df["n"] = anova_df.groupby(TREAT_COL)["y"].count().reindex(means_df[TREAT_COL]).values
                # 순서 통일
