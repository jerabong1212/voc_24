...
def _lazy_import_stats():
    ...
    smf = importlib_import_module = importlib.import_module("statsmodels.formula.api")
        ...
        if chart_type == "막대그래프":
            ...
            if show_anova:
                ...
                if anova_df[TREAT_COL].nunique() >= 2 and all(anova_df.groupby(TREAT_COL)[selected_voc_internal].count() >= 2):
                    ...
                    st.info("ANOVA를 위한 표본 수가 부족합니다. (처리≥2, 각 처리 n≥2 권장)")
...
# ----- 전체 VOC 스크리닝 -----
    ...
    for voc in voc_columns:
        ...
        results.append({"VOC": display_name(voc), "p_value": pval, "Significance": stars})
